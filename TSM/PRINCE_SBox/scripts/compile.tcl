...
set TOPCELL_ENT PRINCE_sbox_opt
set TOPCELL_ARCH verilog

# Analyze target design
# $REPO_TOP/PRINCE_SBox/rtl/ refers to the directory where the rtl files(including the top module) are stored
# Replace $REPO_TOP with the local path where the repository is cloned
analyze -library WORK -autoread {$REPO_TOP/PRINCE_SBox/rtl/} > ./logfiles/analyze.log
...
# Replace $REPO_TOP with the local path where the repository is cloned
write -format verilog -hierarchy -output $REPO_TOP/PRINCE_SBox/netlist/compile_ultra/PRINCE_sbox_opt_compile_ultra.v
